...
def main(continue_train, train_time, train_epoch):
    ...
    embedding_model, Dense_mu_sigma_model, model_name = get_gan(dataset.num_tokens)

    model_dataset = model_name + '-' + dataset.name

    train_dataset = dataset.get_train_dataset()
    pic = draw(10, temp_root, model_dataset, train_time=train_time)
    Stage1_generator_optimizer = tf.keras.optimizers.Adam(1e-4, beta_1=0.5)
    Stage1_discriminator_optimizer = tf.keras.optimizers.Adam(1e-4, beta_1=0.5)
    Stage2_generator_optimizer = tf.keras.optimizers.Adam(1e-4, beta_1=0.5)
    ...
    embedding=embedding_model, Dense_mu_sigmamodel=Dense_mu_sigma_model)
    ...
    train = train_one_epoch(model=[Stage1_generator, Stage1_discriminator, Stage2_generator
        , Stage2_discriminator, embedding_model, Dense_mu_sigma_model],
    ...
    for epoch in range(train_epoch):
        ...
        if (epoch + 1) % 5 == 0:
            ckpt_manager.save()
        pic.save_created_pic([Stage1_generator, Stage2_generator, embedding_model, Dense_mu_sigma_model],
