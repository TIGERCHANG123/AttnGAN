...
def  mixed_precision_main(continue_train, train_time, train_epoch, mid_epoch, ckpt_index=-1):
    ...
    lr = 2e-4
    Stage1_generator_optimizer = tf.keras.optimizers.Adam(lr, beta_1=0.5)
    Stage1_generator_optimizer = tf.keras.mixed_precision.experimental.LossScaleOptimizer(Stage1_generator_optimizer, loss_scale='dynamic')
    Stage1_discriminator_optimizer = tf.keras.optimizers.Adam(lr, beta_1=0.5)
    Stage1_discriminator_optimizer = tf.keras.mixed_precision.experimental.LossScaleOptimizer(Stage1_discriminator_optimizer, loss_scale='dynamic')
    Stage2_generator_optimizer = tf.keras.optimizers.Adam(lr, beta_1=0.5)
    Stage2_generator_optimizer = tf.keras.mixed_precision.experimental.LossScaleOptimizer(Stage2_generator_optimizer, loss_scale='dynamic')
    Stage2_discriminator_optimizer = tf.keras.optimizers.Adam(lr, beta_1=0.5)
    Stage2_discriminator_optimizer = tf.keras.mixed_precision.experimental.LossScaleOptimizer(Stage2_discriminator_optimizer, loss_scale='dynamic')

    checkpoint_path = temp_root + '/temp_model_save/' + model_dataset
    embedding_checkpoint_path = './embedding_model/' + dataset.name
    ckpt = tf.train.Checkpoint(Stage1_genetator_optimizer=Stage1_generator_optimizer,
                               Stage1_discriminator_optimizer=Stage1_discriminator_optimizer,
                               Stage2_genetator_optimizer=Stage2_generator_optimizer,
                               Stage2_discriminator_optimizer=Stage2_discriminator_optimizer,
                               Generator=Generator, Discriminator=Discriminator,
                               Dense_mu_sigma_model=Dense_mu_sigma_model, embedding_model=embedding_model)
    embedding_ckpt = tf.train.Checkpoint(embedding=embedding_model)
    ...
    if ckpt_manager.latest_checkpoint and continue_train:
        print('len ckpts: ', len(ckpt_manager.checkpoints))
        print('Latest checkpoint restored!!')
        ckpt.restore(ckpt_manager.checkpoints[ckpt_index])
    ...
    train = train_one_epoch(model=[Generator, Discriminator, embedding_model, Dense_mu_sigma_model],
                            optimizers=[Stage1_generator_optimizer, Stage1_discriminator_optimizer,
                                        Stage2_generator_optimizer, Stage2_discriminator_optimizer],
                            train_dataset=train_dataset, metrics=[gen_loss, disc_loss], noise_dim=noise_dim, gp=20)
...
if __name__ == '__main__':
    ...
    mixed_precision_main(continue_train=continue_train, train_time=train_time, train_epoch=epoch, mid_epoch=mid_epoch)
